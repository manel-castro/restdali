...
router.get(
  ...
  async function (req: Request, res: Response, next: NextFunction) {
    ...
    const existinProject = await prisma.project.findFirst({
      where: { domain },
      include: {
    ...
    const favicon = existinProject?.generalPageContent.favicon;
    const pageTitle = existinProject?.generalPageContent.pageTitle;
    const paginasOrder = existinProject?.paginasOrder;

    const pageNames = existinProject?.paginas.map((pagina) => ({
      name: pagina.name,
      translations: pagina.translations,
    }));
